...
$_SESSION['varname'] = $cust_id;//
 ...
 $query0 = "Select cust_fname,cust_lname from customer where cust_id = $cust_id";
			   $mysqli = NEW Mysqli("localhost","root","dbproject","gameshop");
			   $results0 = mysqli_query($mysqli,$query0);

				foreach ($results0 as $customer)
			   {
			   	echo'<p align=right>'. "You are logged in as:" . $customer['cust_fname'] . " " . $customer['cust_lname'].'</p>';
			   }
?>
	...
	$resultSet = $mysqli->query("SELECT product_name,unit_price_pur, unit_price_rent,rating,year, director,actor_name,genre,availability
		...
		if($resultSet->num_rows>0){
			...
			while($row = $resultSet->fetch_assoc())
			{
				...
				echo '<tr><td align="left"><a href="rating.php?var=' . $pname.  '"/>' .  
